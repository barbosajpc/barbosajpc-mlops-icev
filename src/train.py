...
def main() -> None:
    ...
    pipelines = [("RandomForest", pipe_rf), ("GradientBoosting", pipe_gb)]

    for nome, clf in pipelines:
        with mlflow.start_run(run_name=f"{nome}-pipeline") as run:
            ...
            mlflow.sklearn.log_model(
                sk_model=clf,
                name=nome,
                registered_model_name=f"{MODEL_NAME}-{nome}",
            )
